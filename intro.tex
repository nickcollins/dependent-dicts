...
To establish a one-to-one correspondence between association lists and semantic mappings, one solution~\citep{XXX,XXX,XXX} is to maintain a \emph{canonical}
%
\footnote{
  Some treatments, including the Coq standard library \citep{XXX,XXX,XXX}, describe non-canonicalized \sal{}s as ``weak'', implying that \cal{}s are ``strong''.
}
