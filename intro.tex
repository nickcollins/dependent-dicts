...
Consider dictionaries, which---among a broad range of purposes---are used pervasively for implementing type contexts and evaluation environments.
%
...
To establish a one-to-one correspondence between association lists and semantic mappings, a common solution~\citet{XXX,XXX,XXX} is to maintain a \emph{canonical} form that is semantically valid and unique, namely, a list in which there are no duplicate keys and, furthermore, where keys are in sorted order.
%
The second association list described above is one such example.
%
...
%
\rkc{Explain why maybe this isn't so bad...}
%
}
%
...
Furthermore, in addition to properties about the external interface of the data structure, it is often useful to retain the ability to inspect, iterate, and manipulate the internal representation.
%
...
\autoref{fig:prop-summary} summarizes the preceding discussion along these dimensions; note that all representations except partial functions (without keys) can be easily destructed.
%
...
%
We offer a solution, dubbed \emph{\dds{}}, which (at least partially) achieves the four desired properties. 
%
...
%
Every unique \dd{} represents a unique semantic mapping, thus built-in equality may be used (\SemInj).
%
%% there is a bijection between \dd{} terms and finite maps,
%
Furthermore, if two \dds{} are semantically equal, they must also be reflexively equal (\EqDec).
...
%
% TODO: Okay to oversimplify here?
%
Furthermore, compared to each of the conventional representations, our implementation of deltas requires that the key type not merely be ordered but also be isomorphic to the naturals.
%
%% Furthermore, compared to each of the conventional representations, the use of deltas requires that the key type not merely be ordered but also possess a difference operation and a least element.
%
...
%% Naturally, {\dd}s have some drawbacks as well.
